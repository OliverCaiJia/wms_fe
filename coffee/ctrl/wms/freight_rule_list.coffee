'use strict'

angular.module 'app.controllers'

.controller 'WmsFreightRuleListCtrl', ($scope, $routeParams, $filter, CommonService, WmsFreightRuleService, ClickEditService) ->
  $scope.select = (page) ->
    start = (page - 1) * $scope.searchOption.numPerPage
    end = start + $scope.searchOption.numPerPage
    $scope.currentPageItems = $scope.filteredItems.slice(start, end)

  $scope.onFilterChange = ->
    $scope.select(1)
    $scope.currentPage = 1
    $scope.row = ''

  $scope.onNumPerPageChange = ->
    $scope.select(1)
    $scope.currentPage = 1

  $scope.onOrderChange = ->
    $scope.select(1)
    $scope.currentPage = 1

  $scope.search = ->
    $scope.filteredItems = $filter('filter')($scope.items, $scope.searchOption.keywords)
    $scope.onFilterChange()

  # orderBy
  $scope.order = (rowName)->
    if $scope.row == rowName
      return
    $scope.row = rowName
    $scope.filteredItems = $filter('orderBy')($scope.filteredItems, rowName)
    $scope.onOrderChange()

  # initSearch
  initSearch = ->
    $scope.searchOption = {"keywords":""}
    $scope.filteredItems = []
    $scope.row = ''
    $scope.numPerPageOpt = [10, 20, 50, 100]
    $scope.searchOption.numPerPage = $scope.numPerPageOpt[0]
    $scope.currentPage = 1
    $scope.currentPageItems = []
    $scope.search()
    $scope.select($scope.currentPage)
        
  # add
  $scope.add = ->
    promise = WmsFreightRuleService.add($scope.freightRule)
    promise.then (data) ->
      $scope.freightRule.id = data
      $scope.items.unshift($scope.freightRule)
      $scope.createFreightRule = {}
      $scope.freightRule = {}
      initSearch()
                
  # delete
  $scope.delete = (id, i) ->
    params = {id: id}
    promise = WmsFreightRuleService.delete(params)
    promise.then ->
      $scope.items.splice(i, 1)
      initSearch()
                
  # update
  $scope.updateAddedWeightPrice = (freightRule, fieldName) ->
    if ClickEditService.updateNode(freightRule, fieldName)
      promise = WmsFreightRuleService.updateAddedWeightPrice(freightRule)
      promise.then ->
                
  # update
  $scope.updateFirstWeightPrice = (freightRule, fieldName) ->
    if ClickEditService.updateNode(freightRule, fieldName)
      promise = WmsFreightRuleService.updateFirstWeightPrice(freightRule)
      promise.then ->
                
  $scope.switchCollapse = (node) ->
    CommonService.switchCollapse(node)

  $scope.switchNode = (freightRule, fieldName, bool) ->
    ClickEditService.switchNode(freightRule, fieldName, bool)

  prepareConst = ->

  main = ->
    $scope.createFreightRule = {}
    $scope.freightRule = {}
    prepareConst()

  main()