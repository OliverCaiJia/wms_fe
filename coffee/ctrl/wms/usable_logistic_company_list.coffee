'use strict'

angular.module 'app.controllers'

.controller 'WmsUsableLogisticCompanyListCtrl', ($scope, $routeParams, $filter, CommonService, WmsUsableLogisticCompanyService, ClickEditService) ->
  $scope.select = (page) ->
    start = (page - 1) * $scope.searchOption.numPerPage
    end = start + $scope.searchOption.numPerPage
    $scope.currentPageItems = $scope.filteredItems.slice(start, end)

  $scope.onFilterChange = ->
    $scope.select(1)
    $scope.currentPage = 1
    $scope.row = ''

  $scope.onNumPerPageChange = ->
    $scope.select(1)
    $scope.currentPage = 1

  $scope.onOrderChange = ->
    $scope.select(1)
    $scope.currentPage = 1

  $scope.search = ->
    $scope.filteredItems = $filter('filter')($scope.items, $scope.searchOption.keywords)
    $scope.onFilterChange()

  # orderBy
  $scope.order = (rowName)->
    if $scope.row == rowName
      return
    $scope.row = rowName
    $scope.filteredItems = $filter('orderBy')($scope.filteredItems, rowName)
    $scope.onOrderChange()

  # initSearch
  initSearch = ->
    $scope.searchOption = {"keywords":""}
    $scope.filteredItems = []
    $scope.row = ''
    $scope.numPerPageOpt = [10, 20, 50, 100]
    $scope.searchOption.numPerPage = $scope.numPerPageOpt[0]
    $scope.currentPage = 1
    $scope.currentPageItems = []
    $scope.search()
    $scope.select($scope.currentPage)
        
  # add
  $scope.add = ->
    promise = WmsUsableLogisticCompanyService.add($scope.usableLogisticCompany)
    promise.then (data) ->
      $scope.usableLogisticCompany.id = data
      $scope.items.unshift($scope.usableLogisticCompany)
      $scope.createUsableLogisticCompany = {}
      $scope.usableLogisticCompany = {}
      initSearch()
                
  # delete
  $scope.delete = (id, i) ->
    params = {id: id}
    promise = WmsUsableLogisticCompanyService.delete(params)
    promise.then ->
      $scope.items.splice(i, 1)
      initSearch()
                
  # update
  $scope.updateCode = (usableLogisticCompany, fieldName) ->
    if ClickEditService.updateNode(usableLogisticCompany, fieldName)
      promise = WmsUsableLogisticCompanyService.updateCode(usableLogisticCompany)
      promise.then ->
                
  # update
  $scope.updateName = (usableLogisticCompany, fieldName) ->
    if ClickEditService.updateNode(usableLogisticCompany, fieldName)
      promise = WmsUsableLogisticCompanyService.updateName(usableLogisticCompany)
      promise.then ->
                
  $scope.switchCollapse = (node) ->
    CommonService.switchCollapse(node)

  $scope.switchNode = (usableLogisticCompany, fieldName, bool) ->
    ClickEditService.switchNode(usableLogisticCompany, fieldName, bool)

  prepareConst = ->

  main = ->
    $scope.createUsableLogisticCompany = {}
    $scope.usableLogisticCompany = {}
    promise = WmsUsableLogisticCompanyService.listAll()
    promise.then (data) ->
      $scope.items = data
      initSearch()
                
    prepareConst()

  main()