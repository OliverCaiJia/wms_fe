...
.controller 'WmsIncreaseAvailableContainerQuantityCtrl', ($scope, $routeParams, WmsContainerService,$i18next
  WmsMemberService,WmsSellerAvailableContainerService,WmsAvailableContainerInventoryLogService,WmsSellerService,ModalService) ->
  $scope.select = (page) ->
  ...
  $scope.increaseInventory = (availableContainerInventoryLog) ->
    if (parseInt($scope.container.postInventory) - parseInt($scope.availableContainerInventoryLog.number)) < 0
      ModalService.showMessageOnError($i18next("wms:ui.label.postInventory") + $i18next("wms:message.inventoryIsNegative"))
    else
